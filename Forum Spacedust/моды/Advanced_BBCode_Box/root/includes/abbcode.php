...
class abbcode
{
	...
	**/
	function abbcode_init( )
	{
		...
		$this->abbcode_config = array(
			...
			'ABBC3_RESIZE'			=> $config['ABBC3_RESIZE'],
			...
			// 'enlarge' 'samewindow' 'newwindow' 'greybox'
	...
	**/
	...
	/**
	* 109 Check some bbcodes permissions status
	*
	...
	**/
	...
	/**
	* 109 Parsing the e-links  - Second pass.
	* 
	...
	**/
	function ed2k_pass( $bbcode_id, $var1, $var2)
	{
		...
		if ( $matches )
		{
			foreach ( $match[0] as $i => $m )
			{
				...
				return $ed2k_icon . '&nbsp;' . '<a href="' . $ed2k_link . '" class="postlink">' . $ed2k_name . '</a>&nbsp;[' .humanize_size($ed2k_size) . ']&nbsp;' . $ed2k_stat;
			}
		}
		else
		{
	...
	/**
	* 109 Parsing the serch text - Second pass.
	*
	...
	**/
	...
	/**
	* 109 Parsing thumbnail images - Second pass.
	*
	* @param string		$stx	align (right|center|left)
	* @param string		$in		URL = post text between [img=(right|center|left)] & [/img]
	* @return string	image
	**/
	function thumb_pass( $stx, $in )
	{
		...
		switch ( $stx )
		{
			case 'left':
			case 'right':
				return '<div style="text-align:' . $stx . '"><a href="' . $in . '" rel="gb_imageset[]" alt="' . $user->lang['IMAGE'] . '" title="" class="hoverbox" onclick="externalLinks(this);"><img src="' . $in . '" border="0" width="' . $config['ABBC3_MAX_THUM_WIDTH'] . '"/></a></div>';
				break;
			case 'center':
				return '<center><a href="' . $in . '" rel="gb_imageset[]" alt="' . $user->lang['IMAGE'] . '" title="" class="hoverbox" onclick="externalLinks(this);"><img src="' . $in . '" border="0" width="' . $config['ABBC3_MAX_THUM_WIDTH'] . '" align="middle" /></a></center>';
				break;
			default:
				return '<a href="' . $in . '" rel="gb_imageset[]" alt="' . $user->lang['IMAGE'] . '" title="" class="hoverbox" onclick="externalLinks(this);"><img src="' . $in . '" border="0" width="' . $config['ABBC3_MAX_THUM_WIDTH'] . '"/></a>';
				break;
		}
	}
	
	/**
	* 109 Parsing the images aligned - Second pass.
	*
	* @param string		$stx	align (right|center|left)
	...
	**/
	...
	{
		return '<div align="' . $stx . '"><img src="' . $in . '" alt="" onload="NcodeImageResizer.createOn(this);" /></div>';
	...
	/**
	* 109 Parsing the NFO text - Second pass.
	*
		...
		foreach ($linktest_links as $linktest_value)
		{
			if (trim($linktest_value) !== '')
			{
				...
				if (!$linktest_return)
				{
					...
					$linktest_pic	= "<img src='" . $this->abbcode_config['ABBC3_PATH'] . "/images/error.gif' class='postimage' type='image' name='abbcode_linktest' alt='$linktest_wrong' title='$linktest_wrong' />&nbsp;";
				}
				else
				{
					$linktest_msg	= "<font color='green' size='4' >$linktest_good</font>";
					$linktest_pic	= "<img src='" . $this->abbcode_config['ABBC3_PATH'] . "/images/ok.gif'    class='postimage' type='image' name='abbcode_linktest' alt='$linktest_good'  title='$linktest_good'  />&nbsp;";
				}
	...
	**/
	...
	/**
	* 109 Count the number of click on a link or image
	*
...
//			'youtube.com'		=> array ( 'display' => true	,'image' => 'youtube.gif'		,'example' => "http://mx.youtube.com/watch?v=TA4hm97m494&rel=1"																			,'found' => "#http://((.*?)?)youtube.com/watch\?v=([0-9A-Za-z-_]+)?([^[]*)?#is"							,'regexp' => '<object width="{WIDTH}" height="{HEIGHT}"><param name="movie" value="http://$2youtube.com/v/$3" /><param name="wmode" value="transparent"/><embed src="http://$2youtube.com/v/$3" type="application/x-shockwave-flash" wmode="transparent" width="{WIDTH}" height="{HEIGHT}"></embed></object><br/>' ),
	...
	**/
			...
			if ( ( preg_match( '#' . $video_name . '#si', $in ) ) && ( $video_data['display'] ) )
			{
				...
				if ( $video_data['found'] != '' && $video_data['regexp'] != '' )
				{
					if ( preg_match( $video_data['found'], $in ) )
					{
						...
						{
							$img = '<img src="' . $this->abbcode_config['ABBC3_PATH'] . '/images/' . $video_data['image'] .'" class="postimage" type="image" alt="' . $video_name . '" title="' . $video_name . '" /> <strong>' . $user->lang['ABBC3_BBVIDEO_VIDEO'] .' : <a href="http://' . $video_name . '" target="_blank" >' . preg_replace('#\((.*?)\)#is','',$video_name) . '</a></strong>';
						}
						else
						{
							$img = '<img src="' . $this->abbcode_config['ABBC3_PATH']  . '/images/' . $video_data['image'] .'" class="postimage" type="image" alt="' . $video_name . '" title="' . $video_name . '" /> <strong>' . $user->lang['ABBC3_BBVIDEO_FILE'] .' : </strong>' . $video_name;
						}
	...
	/**
	* 109 Parsing the web videos without a clear link.
	*
	* @param string		$in		post text between [BBvideo] & [/BBvideo]
...
// End of abbcode3 class
	...
	*/
	...
	function abbcode_specialchars2($text)
	{
		$str_from = array("'"	,'"'	,'<'	,'>'	,'['	,']'	,'.'	,':');
		$str_to   = array("\'"	,'\"'	,'&lt;'	,'&gt;'	,'&#91;','&#93;','&#46;','&#58;');
		return str_replace($str_from, $str_to, $text);
